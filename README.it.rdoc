= Sinatra
<i>Attenzione: questo documento è una traduzione dell'originale inglese e
potrebbe non essere aggiornato.</i>

Sinatra è un DSL per creare velocemente applicazioni web in Ruby con 
minimo sforzo:
...
Installa la gemma ed esegui lo script:

  gem install sinatra
  ruby -rubygems miaapp.rb

Visualizza il risultato in: http://localhost:4567

Si raccomanda di eseguire anche <tt>gem install thin</tt>, che verrà 
utilizzato da Sinatra se disponibile.

== Routes

In Sinatra, una routa è un metodo HTTP accoppiato con un pattern che
riconosce un URL. Ogni route è associata ad un blocco:
...
Le route vengono riconosciute nell'ordine in cui sono definite.
Viene attivata sempre la prima route che corrisponde alla richiesta.

I route pattern possono includere dei parametri, accessibili attraverso
l'hash <tt>params</tt>:

  get '/hello/:nome' do
    # riconosce "GET /hello/foo" e "GET /hello/bar"
    # params[:nome] sarà 'foo' oppure 'bar'
    "Hello #{params[:nome]}!"
  end

È possibile accedere ai parametri utilizzando i blocchi:

  get '/hello/:nome' do |n|
    "Hello #{n}!"
  end

I route pattern possono anche includere parametri variabili (wildcard),
accessibili attraverso l'array <tt>params[:splat]</tt>:

  get '/say/*/to/*' do
    # riconosce /say/hello/to/world
    params[:splat] # => ["hello", "world"]
  end

  get '/download/*.*' do
    # riconosce /download/percorso/al/file.xml
    params[:splat] # => ["percorso/al/file", "xml"]
  end

Oppure attraverso parametri di blocco:

  get '/download/*.*' do |path, ext|
    [path, ext] # => ["percorso/al/file", "xml"]
  end

Route matching utilizzando espressioni regolari:
...
I route pattern possono avere parametri opzionali:

  get '/posts.?:format?' do
    # riconosce "GET /posts" e tutte le estensioni "GET /posts.json", "GET /posts.xml" ecc.
  end

Inoltre, a meno che la protezione dall'attacco di attraversamento del percorso 
(vedi sotto) non sia disabilitata, il percorso di richiesta potrebbe essere modificato prima
di essere testato rispetto alle route specificate.

=== Condizioni

Le route possono includere anche condizioni di corrispondenza, ad esempio lo 
user agent:

  get '/foo', :agent => /Songbird (\d\.\d)[\d\/]*?/ do
    "Stai usando Songbird versione #{params[:agent][0]}"
  end

  get '/foo' do
    # Riconosce i browser diversi da Songbird
  end

Altre condizioni disponibili sono +host_name+ e +provides+:

  get '/', :host_name => /^admin\./ do
    "Admin Area, Accesso Negato!"
  end
...
È possibile definire altri tipi di condizioni:

  set(:probabilita) { |value| condition { rand <= value } }
  
  get '/vinci_auto', :probabilita => 0.1 do
    "Hai vinto!"
  end
...
Per una condizione che accetta valori multipli si deve utilizzare un asterisco:

  set(:auth) do |*roles|   # <- nota l'uso dell'asterisco
    condition do
  ...
  get "/only/admin/", :auth => :admin do
    "Solo gli amministratori sono ammessi qui!"
  end
  
=== Valori di ritorno

Il valore di ritorno di un blocco di una route determina almeno il corpo di 
rispota restituito al client HTTP, o almeno il prossimo middleware nello stack 
Rack.
Di solito, il valore di ritorno è una stringa, come negli esempi precedenti. 
Ma sono possibili anche altri valori.

È possibile restituire qualsiasi oggetto che sia una valida risposta Rack, 
un oggetto corpo Rack oppure uno codice di stato HTTP:

* Un Array con tre elementi: <tt>[status (Fixnum), headers (Hash), response
  body (responds to #each)]</tt> 
* Un Array con due elementi: <tt>[status (Fixnum), response body (responds to
  #each)]</tt>                 
* Un oggetto che risponde a <tt>#each</tt> e passa solo stringhe al blocco dato 
* Un Fixnum rappresentante lo status code

In questo modo è possibile, ad esempio, implementare facilmente un esempio 
di streaming:
...
È anche possibile utilizzare il metodo helper +stream+ (descritto più avanti) 
in modo da ridurre il codice ripetuto e incorporare la logica di streaming 
nella route.
